...
The XLSX workbook is provided for easier review/editing in Excel when selected. The current v0.2.0 builder reads the CSV files when writing the USB.
...
## v0.2.0 local install and download/install infrastructure
...
v0.2.0 adds an explicit download manager. It uses `ToolList_SOURCE_OF_TRUTH.csv` as the source of truth. A row must have `Include=Yes`, `DownloadEnabled=Yes`, and a populated `DirectDownloadURL` before the tool attempts a download.
